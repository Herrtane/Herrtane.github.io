...
> **파이썬 `int` 객체는 “값 하나”가 아니라 꽤 큰 구조체라서, 메모리에 그대로 넣으면 완전히 다른 모양이 된다**

### 순수 바이트

pwning에서 원하는 건 아래와 같다.
...
실제로 메모리 상에는 이런 느낌으로 들어가게 된다. (최소 28~32바이트 이상)

```text
08 00 00 00 00 00 00 00   # refcount
?? ?? ?? ?? ?? ?? ?? ??   # type pointer (PyLongObject)
01 00 00 00 00 00 00 00   # size (digit 개수)
55 00 00 00 00 00 00 00   # 실제 값
```
