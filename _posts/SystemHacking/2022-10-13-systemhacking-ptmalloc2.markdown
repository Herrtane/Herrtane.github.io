---
layout: post
title: <System Hacking> 20. Heap, 그리고 ptmalloc2 (2026.05.14 수정)
...
아래 그림은 Chunk의 구조를 그림으로 나타낸 것이다.
...
```
                         ...
                         0x8             0x10                      0x8             0x10 
  ┌──  ┌──────────────────┬────────┬──┬──┬──┐    ┌──────────────────┬────────┬──┬──┬──┐ 
  │    │     prev_size    │  size  │A │M │P │    │     prev_size    │  size  │A │M │P │ 
  │    ├──────────────────┴────────┴──┴──┴──┤    ├──────────────────┼────────┴──┴──┴──┤ 
  │    │                                    │    │        fk        │       bk        │ 
  │    │                                    │    ├──────────────────┴─────────────────┤ 
Size   │                                    │    │                                    │ 
...
3. flags : 3비트. 64비트 환경에서 청크는 16바이트 단위로 할당되므로, size의 하위 4비트는 의미를 갖지 않는다. 그래서 size의 하위 3비트를 청크 관리에 필요한 flag 값으로 사용한다. (바로 밑에서 설명) 각 flag는 순서대로 A(Non_main_Arena, 0x4), M(Is_Mmaped, 0x2), P(Prev-in-use, 0x1)를 나타낸다. **P는 직전 청크가 사용중(할당중)이거나, 혹은 free된 chunk가 fastbin에 들어가는 경우 1을 나타내므로, 이 플래그를 참조하여 0x0일 경우 병합이 필요하다고 판단**할 수 있다.  
4. fd, bk : 각각 8바이트. 연결 리스트에서 다음, 이전 청크를 가리킨다. 해제된 청크에만 존재한다.

<br/>

여기서 size의 하위 4비트는 의미를 갖지 않는다는게 무슨 의미일까? 아래를 보면 이해할 수 있다.
