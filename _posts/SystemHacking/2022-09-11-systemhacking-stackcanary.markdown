---
layout: post
title: <System Hacking> 10. BOF 보호기법 (1) - Stack Canary (2026.04.29 수정)
...
TLS는 Thread Local Storage의 약자로, 각 스레드마다 따로 가져야 하는 전역 비슷한 변수를 저장하는 공간이다. 저장되는 데이터의 예시로는 다음과 같은 것들이 있다.

- TCB (Thread Control Block, 내가 OS에서 배운 그것)
- errno
- 스택 카나리
- thread id
- glibc 내부 데이터

TLS는 “스레드 전용 저장 공간 전체”를 말하는 개념이고, 그 TLS 영역의 맨 앞에 있는 핵심 관리 구조체가 TCB다.
또한, 리눅스에서는 FS 레지스터가 TLS의 시작 주소(base address)를 가리킨다. 그래서 이런 코드가 가능하다.

```s
mov rax, QWORD PTR fs:0x28
```

TLS base + 0x28 위치에 있는 값을 읽는 코드이며, 이것이 바로 __stack_chk_guard (카나리 값)이다.
...
정확히는, 64bit기준으로 canary값의 랜덤 생성 부분은 7바이트이고, 리눅스에서는 첫 바이트가 널 바이트인, **"\x00 + 7바이트의 임의의 값"**으로 구성된다. 그 이유가 궁금해서 찾아보니, gets(), strcpy()등의 string 관련 함수들이 널바이트를 만나면 종료되는 특성을 이용해서, 공격자가 canary를 알기위해 손쉽게 leak 하는 것을 어느정도 방지하기 위함이라고 하더라.
